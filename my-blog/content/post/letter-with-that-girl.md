...
# 创建“霖”字的模板图片
def create_mask_image(char, font_path='msyh.ttc', size=1600):  
    image = Image.new('RGB', (size, size), 'white')
    draw = ImageDraw.Draw(image)
    font = ImageFont.truetype(font_path, int(size * 0.9))
    text_bbox = draw.textbbox((0, 0), char, font=font)
    text_width = text_bbox[2] - text_bbox[0]
    text_height = text_bbox[3] - text_bbox[1]
    draw.text(((size - text_width) / 2, (size - text_height) / 2), char, font=font, fill='black')
    mask = np.array(image.convert('L'))
    mask = np.where(mask > 128, 255, 0)
    return mask

# 生成词云
def generate_wordcloud(text, mask, output_image_path):
...
#注意文件和代码同目录
def main():
    markdown_file_path = 'letter-with-that-girl.md'  # 替换为你自己的Markdown文件路径
    output_image_path = 'wordcloud.png'  # 输出的词云图片路径

    # 读取并处理文本
    text = read_markdown(markdown_file_path)
    chinese_text = extract_chinese(text)
    processed_text = chinese_word_segmentation(chinese_text)

    # 创建“霖”字的模板图片
    mask_image = create_mask_image('霖')

    # 生成词云
    generate_wordcloud(processed_text, mask_image, output_image_path)
...
![丫头](https://picx.zhimg.com/80/v2-c5dae2cbe4877fb881f123eadb5a762f_1440w.png)
